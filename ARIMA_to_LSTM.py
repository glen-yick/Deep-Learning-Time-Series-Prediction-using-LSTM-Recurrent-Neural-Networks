...
fig.savefig('ARIMA_prediction.jpg', bbox_inches='tight')

fig = plt.figure(figsize=(20, 8))
forecast = results.predict()
rmse = mean_squared_error(train_data['Spot price'], forecast) ** 0.5
plt.plot(forecast)
fig.savefig('spot_price.jpg', bbox_inches='tight')
...
fig = plt.figure()
plt.plot(pd.DataFrame(errordata['error']), label='real error')
plt.xlabel('Date')
plt.ylabel('error')
plt.legend()
plt.show()
fig.savefig('error.jpg', bbox_inches='tight')
CombinedCSV = pd.concat([errordata['error'], vixdata], axis=1)
CombinedCSV.to_csv("merge.csv")

# LSTM part
# load the data
path_to_dataset = 'merge.csv'
sequence_length = 20
...
plt.legend()
plt.show()
fig.savefig('error_prediction.jpg', bbox_inches='tight')
